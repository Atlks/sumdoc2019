...
CREATE TABLE `fs_account_t`  (
  ...
  `account_type` int(11) NULL DEFAULT NULL COMMENT '账号类型  1 QQ 2 支付宝 3 微信 4 银行卡 5 其他',
...
) ENGINE = InnoDB AUTO_INCREMENT = 220284310597423105 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '收款账号表' ROW_FORMAT = Compact;
...
-- ----------------------------
-- Table structure for fs_banlace_date_t
-- ----------------------------
DROP TABLE IF EXISTS `fs_banlace_date_t`;
CREATE TABLE `fs_banlace_date_t`  (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `Total_number_of_people` int(11) NULL DEFAULT NULL,
  `platform_id` varchar(30) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL,
  `datax` date NULL DEFAULT NULL,
  `total_balance` decimal(40, 0) NULL DEFAULT NULL,
  `safebox_total_balance` decimal(40, 0) NULL DEFAULT NULL,
  `creatime` timestamp(6) NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP(6),
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE INDEX `plat_date_uniq`(`platform_id`, `datax`) USING BTREE
...
-- ----------------------------
-- Table structure for fs_param_t
-- ----------------------------
DROP TABLE IF EXISTS `fs_param_t`;
CREATE TABLE `fs_param_t`  (
  `id` int(255) NOT NULL AUTO_INCREMENT,
  `class` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL,
  `method` varchar(255) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL,
  `nextStatSelectId` varchar(200) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NULL DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE
...
-- ----------------------------
-- Table structure for fs_safe_sum_t
-- ----------------------------
DROP TABLE IF EXISTS `fs_safe_sum_t`;
CREATE TABLE `fs_safe_sum_t`  (
  `id` bigint(20) NOT NULL COMMENT '主键id',
  `platform_id` bigint(20) NULL DEFAULT NULL COMMENT '平台Id',
  `date` date NULL DEFAULT NULL COMMENT '汇总时间（年月日）',
  `safe_balance` bigint(20) NULL DEFAULT NULL COMMENT '保险箱余额计总',
  `safe_num` int(11) NULL DEFAULT NULL COMMENT '保险箱人数计总',
  `balance` bigint(20) NULL DEFAULT NULL COMMENT '可用余额计总',
  `create_timestamp` datetime(0) NULL DEFAULT NULL,
  `is_deleted` int(11) NULL DEFAULT NULL,
  `statuss` int(11) NULL DEFAULT NULL,
  `update_timestamp` datetime(0) NULL DEFAULT NULL,
  PRIMARY KEY (`id`) USING BTREE
...
CREATE TABLE `temp`  (
  `id` int(11) NOT NULL,
  `in_time` datetime(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0),
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci ROW_FORMAT = Compact;
...
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `All_table` AS select `information_schema`.`TABLES`.`TABLE_NAME` AS `table_name`,`information_schema`.`TABLES`.`TABLE_COMMENT` AS `table_comment` from `information_schema`.`TABLES` where (`information_schema`.`TABLES`.`TABLE_SCHEMA` = 'platform_finance');

-- ----------------------------
-- View structure for stuts_books
-- ----------------------------
DROP VIEW IF EXISTS `stuts_books`;
CREATE ALGORITHM = UNDEFINED SQL SECURITY DEFINER VIEW `stuts_books` AS select `information_schema`.`COLUMNS`.`COLUMN_NAME` AS `column_name列名`,`information_schema`.`COLUMNS`.`COLUMN_COMMENT` AS `column_comment含义`,`information_schema`.`COLUMNS`.`IS_NULLABLE` AS `is_nullable`,`information_schema`.`COLUMNS`.`DATA_TYPE` AS `data_type数据类型`,`information_schema`.`COLUMNS`.`COLUMN_TYPE` AS `column_type`,`information_schema`.`COLUMNS`.`COLUMN_KEY` AS `column_key` from `information_schema`.`COLUMNS` where ((`information_schema`.`COLUMNS`.`TABLE_SCHEMA` = 'platform_finance') and (`information_schema`.`COLUMNS`.`TABLE_NAME` = 'fs_book_t'));

-- ----------------------------
-- Procedure structure for cursorT
-- ----------------------------
DROP PROCEDURE IF EXISTS `cursorT`;
delimiter ;;
CREATE PROCEDURE `cursorT`()
BEGIN
	#Routine body goes here...
		DECLARE $id VARCHAR(30);
	  DECLARE $余额 VARCHAR(30);
	  DECLARE 游标 CURSOR FOR select id,balance from fs_book_t limit 3;
    DECLARE CONTINUE HANDLER FOR NOT FOUND SET @完成=1; 
		
    set @完成=0; 	OPEN 游标;  REPEAT	 FETCH  游标 INTO $id,$余额;			
...
-- ----------------------------
-- Event structure for job1
-- ----------------------------
DROP EVENT IF EXISTS `job1`;
delimiter ;;
CREATE EVENT `job1`
ON SCHEDULE
EVERY '1' DAY STARTS '2019-10-23 18:08:43'
DO select * FROM fs_book_t limit 1;
;;
delimiter ;

-- ----------------------------
-- Triggers structure for table fs_banlace_date_t
-- ----------------------------
DROP TRIGGER IF EXISTS `datex`;
delimiter ;;
CREATE TRIGGER `datex` BEFORE INSERT ON `fs_banlace_date_t` FOR EACH ROW set new.datax=date(now())
;;
delimiter ;
